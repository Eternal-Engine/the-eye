...
async def test_email_is_taken(test_pool, test_user):
    ...
    async with test_pool.acquire() as conn:
        is_email_taken = await check_email_is_taken(users_repo=users_repo.UsersRepository(conn), email=test_user.email)
...
async def test_email_is_not_taken(test_pool):
    ...
    async with test_pool.acquire() as conn:
        is_email_taken = await check_email_is_taken(
            users_repo=users_repo.UsersRepository(conn), email="email.still.available@test.com"
        )
...
async def test_username_is_taken(test_pool, test_user):
    ...
    async with test_pool.acquire() as conn:
        is_username_taken = await check_username_is_taken(
            users_repo=users_repo.UsersRepository(conn), username=test_user.username
        )
...
async def test_username_is_not_taken(test_pool):
    ...
    async with test_pool.acquire() as conn:
        is_email_taken = await check_username_is_taken(
            users_repo=users_repo.UsersRepository(conn), username="availableusername"
        )
...
async def test_user_authentication_failed_with_no_user_found(test_pool):
    ...
    async with test_pool.acquire() as conn:
        is_user_authenticated = await authenticate_user(
            users_repo=users_repo.UsersRepository(conn), email="invalid.email@invaliden.com", password="fake-password"
        )
...
async def test_user_authentication_failed_with_unmatched_password(test_pool, test_user):
    ...
    async with test_pool.acquire() as conn:
        is_user_authenticated = await authenticate_user(
            users_repo=users_repo.UsersRepository(conn), email=test_user.email, password="fake-password"
        )
...
async def test_user_successfully_authenticated(test_pool, test_user):
    ...
    async with test_pool.acquire() as conn:
        is_user_authenticated = await authenticate_user(
            users_repo=users_repo.UsersRepository(conn), email=test_user.email, password="password-test"
        )
